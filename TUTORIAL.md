...
```
Defines `adsorbate` and `surface geometry`. Set `surface geometry` to `None` if adsorbate is already adsorbed. Set `surface` to `None` if interstitial adsorption and inward diffusion is not being studied. Example:
