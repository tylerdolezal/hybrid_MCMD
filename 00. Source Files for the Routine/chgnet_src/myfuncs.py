...
# Load CHGNet model and StructOptimizer
chgnet = CHGNet.load()
...
def relax(system, move_type, supcomp_command):
    ...
    if move_type == 'new_host':
        # Perform full relaxation for interstitial moves
        result = relaxer.relax(structure, steps=1200, fmax=0.1)
    else:
        # Perform rough relaxation for metal-metal swap
        result = relaxer.relax(structure, steps=10, fmax=0.1)


    # Grab the final structure and convert to ASE atoms
    structure = result["final_structure"]
    system = AseAtomsAdaptor.get_atoms(structure)
    write("CONTCAR", system, format="vasp", direct=True, sort=True)

    E = result["trajectory"].energies[-1]
...
def initial_relax(system, supcomp_command):
    ...
    iterative_relax = [(500, 0.5), (2500, 0.1)]
    ...
    for relax in iterative_relax:
        ...
        result = relaxer.relax(structure, steps=relax[0], fmax=relax[1])
...
def grab_md_params():
    """
    Extracts the total number of steps and temperature settings from a LAMMPS input file.

    Args:
        file_path (str): Path to the LAMMPS input file.

    Returns:
        dict: A dictionary with 'steps' and 'temperature' keys.
    """
    with open("MD.in", 'r') as file:
        content = file.read()

    # Regex for extracting the temperature
    temp_match = re.search(r"fix\s+\d+\s+all\s+nvt\s+temp\s+([\d.]+)\s+([\d.]+)", content)
    if temp_match:
        temp_initial = float(temp_match.group(1))
        temp_final = float(temp_match.group(2))
    else:
        return("Cannot find temperature!")

    # Regex for extracting the number of steps
    steps_match = re.search(r"run\s+(\d+)", content)
    steps = int(steps_match.group(1)) if steps_match else None

    return steps, temp_final
...
def run_md_simulation(system, supcomp_command):
    ...
    system = traj[-1]
    # Get the potential energy of the final structure
